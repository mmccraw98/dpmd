...
int main(int argc, char** argv) {
    if (argc != 8) {
        std::cerr << "Usage: " << argv[0] << " <in_path> <out_path> <n_fire_steps> <avg_pe_target> <avg_pe_diff_target> <phi_increment> <phi_tolerance>" << std::endl;
        return 1;
    }
    std::string in_path = argv[1];
    std::string out_path = argv[2];
    const int n_fire_steps = std::stoi(argv[3]);
    const double avg_pe_target = std::stod(argv[4]);
    const double avg_pe_diff_target = std::stod(argv[5]);
    const double phi_increment = std::stod(argv[6]);
    const double phi_tolerance = std::stod(argv[7]);
