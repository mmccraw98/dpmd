...
int main(int argc, char** argv) {
    if (argc != 7) {
        std::cerr << "Usage: " << argv[0] << " <num_particles_per_system> <dt_scale> <n_steps> <S> <out_path> <packing_fraction>" << std::endl;
        return 1;
    }
    ...
    const double packing_fraction = atof(argv[6]);
    ...
    md::integrators::FIREWall fw(P, dt);
    fw.init();
