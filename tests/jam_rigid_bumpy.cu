...
__global__ void fire_update_kernel(
    ...
    if (power[s] > 0) {  // if moving downhill, increase the inertia
        ...
        if (N_good[s] > N_min) {  // if the system has been moving downhill for enough steps, increase the timestep and mixing ratio
            dt[s] = std::min(dt[s] * f_inc, dt_max[s]);
            alpha[s] *= f_alpha;
        }
        ...
        dt[s] = std::max(dt[s] * f_dec, dt_min[s]);
        alpha[s] = alpha_init;
...
void fire_step(
    ...
    df::DeviceField1D<double>& dt_reverse, df::DeviceField1D<double>& velocity_scale,
    df::DeviceField1D<int>& N_good, df::DeviceField1D<int>& N_bad,
    df::DeviceField1D<double>& dt, df::DeviceField1D<double>& alpha,
    df::DeviceField1D<double>& dt_max, df::DeviceField1D<double>& dt_min,
    double f_inc, double f_dec, double f_alpha, double alpha_init,
    long N_min, long N_bad_max
) {
    ...
    P.update_positions(dt_reverse, 1.0);  // does nothing unless dt_reverse_half[i] != 0.0
...
void minimize_fire(
    ...
    long N_min = 5;
    long N_bad_max = 10;
    double dt_max_scale = 10.0;
    double dt_min_scale = 0.001;

    const int S = P.n_systems();

    df::DeviceField1D<double> dt_max; dt_max.resize(dt.size()); dt_max.copy_from(dt); dt_max.scale(dt_max_scale);
    df::DeviceField1D<double> dt_min; dt_min.resize(dt.size()); dt_min.copy_from(dt); dt_min.scale(dt_min_scale);
    df::DeviceField1D<double> dt_reverse; dt_reverse.resize(S); dt_reverse.fill(0.0);
    df::DeviceField1D<double> velocity_scale; velocity_scale.resize(S);
    df::DeviceField1D<double> alpha; alpha.resize(S); alpha.fill(alpha_init);
    df::DeviceField1D<int> N_good; N_good.resize(S); N_good.fill(0);
    ...
    int step = 0;
    while (step < n_steps) {
        k::fire_step(P, dt_reverse, velocity_scale, N_good, N_bad, dt, alpha, dt_max, dt_min, f_inc, f_dec, f_alpha, alpha_init, N_min, N_bad_max);
        P.compute_pe_total();
        auto B = md::launch::threads_for();
        auto G = md::launch::blocks_for(S);
        CUDA_LAUNCH(k::fire_convergence_kernel, G, B,
...
int main(int argc, char** argv) {
    ...
    k::minimize_fire(P, dt, n_minimization_steps, avg_pe_diff_target, avg_pe_target);
